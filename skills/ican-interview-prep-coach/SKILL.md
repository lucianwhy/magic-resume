...
4. 读取 `references/common/stage-router.md`，识别当前阶段；阶段不明时先给四阶段培训地图，再由用户选择。
...
9. 用户要求视频、课程或外部学习资源时，读取 `references/common/video-learning.md`，再运行 `scripts/search_bilibili_videos.py`。先消除缩写歧义；不要仅按播放量推荐。
